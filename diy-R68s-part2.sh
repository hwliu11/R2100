...
trojan_ver=`grep -n "PKG_VERSION" ./package/trojan-plus/Makefile |awk -F:= '{print $2}'`

if [ ! "$trojan_ver"="" ]; then
### fix gsl
if [[ ! -d "./build_dir/target-aarch64_generic_musl/trojan-plus-$trojan_ver/external/" ]]; then
	mkdir -p ./build_dir/target-aarch64_generic_musl/trojan-plus-$trojan_ver/external
fi
if [[ ! -d "./build_dir/target-aarch64_generic_musl/trojan-plus-$trojan_ver/external/GSL" ]]; then
	echo -e "\033[35m pull GSL source \033[0m"
	git clone --depth 1 https://github.com/Microsoft/GSL build_dir/target-aarch64_generic_musl/trojan-plus-$trojan_ver/external/GSL
	if [[ $? -ne 0 ]]; then
		echo -e "\033[31m pull GSL source failed \033[0m"
	fi
elif [ $update_source -eq 1 ]; then
	echo -e "\033[35m update GSL source \033[0m"
	mydir=$(pwd)
	cd ./build_dir/target-aarch64_generic_musl/trojan-plus-$trojan_ver/external/GSL
	git pull
	if [[ $? -ne 0 ]]; then
		echo -e "\033[31m update GSL source failed \033[0m"
	fi
	cd $mydir
fi
if [ ! -d "./build_dir/target-aarch64_generic_musl/trojan-plus-$trojan_ver/external/GSL" ]; then
